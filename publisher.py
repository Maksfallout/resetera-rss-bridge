...
# ====== НАСТРОЙКИ ИИ ======
CHADGPT_API_URL_FAST = "https://ask.chadgpt.ru/api/public/gpt-5.4-mini"   # для куратора, проверки дублей, промпта картинок
CHADGPT_API_URL_SMART = "https://ask.chadgpt.ru/api/public/gpt-5.4-mini"     # для рерайта (мощнее)
...
19.1 Обязательно добавь в конец статьи после текста 4ый абзац, специально под предложение, точную фразу: "Агентская новость дня."
...
def call_chadgpt(prompt, api_key, url=None):
    ...
    if url is None:
        url = CHADGPT_API_URL_FAST   # по умолчанию — быстрая модель
...
def rewrite_article(article_text, api_key):
    ...
    answer = call_chadgpt(prompt, api_key, url=CHADGPT_API_URL_SMART)
    if not answer:
        return None
