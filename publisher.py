...
- Стиль: simple colorful sketch in a notebook, hand-drawn cartoon, light watercolor over pencil lines, casual doodle style
- Соотношение сторон: 16:9 horizontal
- БЕЗ ТЕКСТА на изображении, без слов, без надписей, без логотипов, без брендов
- БЕЗ узнаваемых лиц реальных людей
- Простая композиция, понятная с первого взгляда
- Лёгкая ирония или метафора, отражающая суть новости

ПРАВИЛА ПРОМПТА:
1. Промпт ТОЛЬКО на английском языке.
2. Длина — 50-150 слов.
3. ОБЯЗАТЕЛЬНО включи в промпт фразы: "no text, no letters, no words, no logos, no brand names"
4. ОБЯЗАТЕЛЬНО включи: "simple colorful sketch in a notebook style, hand-drawn cartoon"
5. Опиши КОНКРЕТНУЮ сцену-метафору, а не абстракции. Например, не "gaming industry crisis", а "a cartoon character looking shocked at a giant price tag on a console box".
6. Не упоминай конкретные названия игр, компаний и платформ — заменяй на нейтральные образы (вместо "PlayStation 5" пиши "a generic gaming console", вместо "Steam Controller" — "a futuristic gamepad").
