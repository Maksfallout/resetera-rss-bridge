...
ПРИНЦИПЫ ХОРОШЕЙ ИЛЛЮСТРАЦИИ:
- УЗНАВАЕМАЯ сцена — читатель за полсекунды должен понять о чём картинка.
- 1-2 конкретных персонажа или предмета в кадре. Не один абстрактный символ, но и не толпа.
- Сцена должна напрямую отражать суть новости — ситуация, эмоция, действие.
- Минималистичный фон — без архитектуры, магазинов, вывесок, толп.
...
- Лица героев — мультяшные, с выраженной эмоцией (удивление, недоумение, радость, шок)

ЖЁСТКИЕ ТРЕБОВАНИЯ К СОДЕРЖАНИЮ:
- АБСОЛЮТНО НИКАКОГО ТЕКСТА: no text, no letters, no words, no signs, no labels, no logos, no readable writing anywhere
- Если в сцене ценник — он должен быть БЕЗ цифр и слов (просто пустой или с восклицательным знаком)
- Если в сцене экран/телевизор — он должен быть пустым или с абстрактными формами, БЕЗ текста
- Если в сцене коробка/упаковка — БЕЗ названий и логотипов
- БЕЗ узнаваемых лиц реальных людей
- БЕЗ конкретных названий игр и компаний — заменяй на нейтральные образы (вместо "PlayStation 5" → "a generic gaming console", вместо "Steam Controller" → "a generic gamepad")

КАК ПРИДУМЫВАТЬ СЦЕНУ:
1. Прочитай заголовок — какая ОДНА главная эмоция или ситуация?
2. Придумай простую сцену с 1-2 элементами, которая её показывает.
3. Не загромождай: один герой + один объект внимания достаточно.

ХОРОШИЕ ПРИМЕРЫ:
- Новость про дороговизну консолей → "a cartoon character with a shocked face holding a tiny gaming console while looking at an oversized empty price tag without any numbers or letters, plain pastel background"
- Новость про задержку игры → "a cartoon character sitting on a giant calendar pointing at one day, looking impatient, simple background"
- Новость про спор разработчиков → "two cartoon characters tugging a single game controller in opposite directions, both with frustrated expressions, plain background"
- Новость про успех игры → "a smiling cartoon character lifting a gigantic generic game controller above the head like a trophy, simple background"

ПЛОХИЕ ПРИМЕРЫ (не делай так):
- "a single coin on white background with letter on it" — слишком абстрактно, нечитаемо
- "a gaming store with shelves and customers" — слишком много элементов, провокация текста
- "an empty room" — нет фокуса, нет эмоции

ПРАВИЛА ПРОМПТА:
- Только английский, длина 60-130 слов
- Опиши: главных героев (1-2), их действие/эмоцию, ключевой объект внимания, фон (всегда минимальный)
